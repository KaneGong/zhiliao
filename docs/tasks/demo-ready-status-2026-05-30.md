...
01 会话完成了线上/本地 Formula Brief 回归：

- `/recommend` 3 个代表问题均能生成结构化 Formula Brief。
- SSE 状态中能看到 `正在整理结构化方案卡片…` 与 `结构化方案卡片已生成`。
- 保存方案 API 级端到端通过。
- `/recipes` 可读回包含 `formula_brief` 和 `trust_score` 的保存结果。
- 未发现横向溢出、卡片挤压、移动端断点错位等阻塞展示问题。
- `npm run verify` 通过。
...
```text
docs/data/demo-question-set-v1.md
```
...
- 婴幼儿配方原料 / 乳铁蛋白查询
- 女性胶原饮与渠道话术风险
- Omega-3 普通食品与保健食品路径分流
- 透明质酸钠法规查询
- 儿童 DHA 声称边界
- 银发日常营养粉

每个问题均包含推荐页面、预期看点、可能风险和演示备注。02 未改 `src/app`、API、prompt，也未新增未索资供应商 JSON。
